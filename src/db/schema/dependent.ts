...
import {
  users, admins, draws, wallets,
  kycSubmissions, notificationCampaigns
} from './core';
...
export const tickets = pgTable('tickets', {
  ...
  ticketNumber: varchar('ticket_number', { length: 50 })
    .notNull()
    .default(sql`'TKT-' || EXTRACT(YEAR FROM now()) || '-' || LPAD(nextval('ticket_number_seq')::text, 6, '0')`),

  pricePaid: numeric('price_paid', { precision: 10, scale: 2 }).notNull(),

  pickedNumbers: varchar('picked_numbers', { length: 200 }),

  isAutoPick: boolean('is_auto_pick')
    .notNull()
    .default(false),
  ...
  drawTicketUnq: uniqueIndex('tickets_draw_ticket_idx').on(t.drawId, t.ticketNumber),
}));
